...
//         try {
//           const response = await chatService.sendMessage(content, language);

//           console.log("Response:", response);
//           if (response.messages && response.messages.length > 0) {
//             aiResponseContent = response.messages[0].content;
...
interface ChatContextType {
  ...
  // Message actions
  sendMessage: (content: string, language?: string) => Promise<void>;
  sendVoiceMessage: (audioFile: File, language?: string) => Promise<void>;
  loadMessages: (sessionId: string) => Promise<void>;
...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const sendVoiceMessage = useCallback(
    async (audioFile: File, language: string = "en") => {
      dispatch({ type: "SET_TYPING", payload: true });
      dispatch({ type: "SET_LOADING", payload: true });
      dispatch({ type: "SET_ERROR", payload: null });
      ...
      try {
        ...
        if (response.audioUrl) {
          ...
          dispatch({ type: "ADD_MESSAGE", payload: textMessage });
        }
      } catch (error) {
        const errorMessage =
